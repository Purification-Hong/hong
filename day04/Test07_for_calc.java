package day04;
/* 입력된 정수 연산식을 계산해서 출력하는 프로그램을 작성하시오.
   예) 100-10    =>90
   방법)
      1. 연산기호의 위치값을 알아낸다.
...
public class Test07_for_calc {
	public static void main(String[] args) {
		...
		for(int i=0; i <len; i++) {
			...
			if(ch<'0' || ch >'9') {   //이 if의 경우는 숫자형태의 문자가 아니므로 연산기호에 해당한다.
				                       // 따라서 이 문자의 위치값을 저장
				idx=i;  //연산 기호를 찾았으므로 반복을 종료
				break;
			}
		}
		...
		int no2=Integer.parseInt(sno2);  // sno2를 sno1으로 잘못 씀.... error
		...
		case '*':
			...
			break;
		case '/':
			result= no1/(double)no2; //정수끼리의 연산의 결과는 정수가 만들어진다
			break;
		case '%':
			result= no1%no2;
			break;
